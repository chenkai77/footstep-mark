 ...
 * @LastEditTime: 2021-10-25 10:03:54
 * @Description: 状态栏扩展
 */
import {
  window,
...
export class WebviewStatusBar {
	private static readonly statusBarShowWebView = statusBarCommandEnum.statusBarShowWebView;
  // statusBar实例
  ...
  async registerFMStatusCommands() {
    let isRegister = await commandIsRegister(WebviewStatusBar.statusBarShowWebView);
    // 如果已经注册了command Key则不再进行注册
    if (isRegister) {
      return;
    }
    commands.registerCommand(WebviewStatusBar.statusBarShowWebView, async () => {
